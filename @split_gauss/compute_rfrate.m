...
w0 = min(-abs(p.dw)-width1, offset-width2);
w1 = max(width1+abs(p.dw), offset+width2);
...
p.rfrate = quad8('split_gauss_spectrum', w0, w1, [], [], lineshape, T2, offset, p.A, p.s2, p.dw)/(2*p.TR);
